...
- **Review before saving.** Every parsed receipt opens in an edit form: fix items, categories and prices, rotate the image, flag warranty items, or discard it on the spot if it's unreadable or a duplicate (nothing is saved and the image is deleted). Already-saved receipts can be deleted the same way from their edit page. The form shows the automatic sum of items next to the total so mismatches are easy to spot (the CLI prints an explicit warning).
- **Duplicate detection.** Re-uploading (or re-importing from a OneDrive folder) the exact same photo is caught by content hash before it ever reaches Gemini — you're taken straight to the receipt that's already saved instead of burning API quota on it again.
